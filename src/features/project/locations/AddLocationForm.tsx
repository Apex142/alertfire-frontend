...
export default function AddLocationForm({ onSuccess, projectId }: AddLocationFormProps) {
  ...
  const { data: currentCompany } = useCompany(userData?.companySelected || '', { realtime: true });
  ...
  const validateStep = (): boolean => {
    switch (step) {
      ...
      case 'details':
        if (formState.locationType === 'new' && (!formState.name.trim() || !formState.address.trim())) {
          notify.error('Veuillez remplir tous les champs obligatoires');
          return false;
        }
        if (formState.locationType === 'saved' && !formState.selectedSavedLocation) {
          notify.error('Veuillez sélectionner un lieu');
          return false;
        }
        if (formState.locationType === 'public' && !formState.selectedPublicLocation) {
  ...
  const getSelectedLocation = () => {
    switch (formState.locationType) {
      case 'public':
        return publicLocations.find(l => l.id === formState.selectedPublicLocation);
      case 'saved':
        return companyLocations.find(l => l.id === formState.selectedSavedLocation);
      default:
        return null;
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ajout du lieu au projet
      console.log('Création du lieu dans le projet', {
        projectId,
        locationData: {
          label: formState.locationType === 'new' ? formState.name.trim() : selectedLocation?.label || '',
          address: formState.locationType === 'new' ? formState.address.trim() : selectedLocation?.address || '',
          notes: formState.locationType === 'new' ? (formState.notes.trim() || undefined) : selectedLocation?.notes,
          locationId: formState.locationType === 'new' ? locationId :
            formState.locationType === 'public' ? formState.selectedPublicLocation :
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex justify-between pt-6 border-t">
        ...
        >
          {loading ? 'Chargement...' :
            step === 'policy' ? 'Ajouter le lieu' : 'Continuer'}
        </Button>
      </div>
    </form>
  );
