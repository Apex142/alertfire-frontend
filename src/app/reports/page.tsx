...
function ReportsContent() {
  ...
  const rows = useMemo<ReportRow[]>(() => {
    const groups = new Map<
      string,
      ReportRow & { tempSum: number; co2Sum: number; confSum: number }
    >();
      ...
      <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
            Analyse détaillée des alertes détectées, exportable pour vos
            équipes.
          </p>
        </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-200/70 px-6 py-4 dark:border-slate-800/60">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {rows.length} ligne{rows.length > 1 ? "s" : ""} générée
              {rows.length > 1 ? "s" : ""}
            </p>
          </div>
        </div>
