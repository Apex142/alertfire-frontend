...
function AnalyticsContent() {
  ...
  const alertsByDay = useMemo(() => {
    ...
    for (let i = daysWindow - 1; i >= 0; i -= 1) {
      ...
      const count = alertsInWindow.filter((alert) => {
        const timestamp = getMillis(alert.timestamp);
        return (
          timestamp !== null && timestamp >= dayStart && timestamp < dayEnd
        );
      }).length;
  ...
  const metrics = useMemo(() => {
    ...
    const confidenceTrend = averageConfidence - previousAverageConfidence;
      ...
      <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
            Vue consolidée des foyers, capteurs et performances sur les{" "}
            {daysWindow} derniers jours.
          </p>
        </div>
      ...
      <section className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-[2fr_1fr]">
              ...
              <div key={bucket.label} className="flex-1">
                ...
                >
                  <div
                    ...
                    style={{
                      height: `${
                        bucket.value === 0 ? 4 : Math.min(100, bucket.value * 8)
                      }%`,
                    }}
                  />
                </div>
        ...
        <Card className="flex flex-col gap-4">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {metrics.offlineProjects} capteur(s) hors ligne et{" "}
            {metrics.activeAlerts} foyer(s) à risque.
          </p>
        </Card>
      </section>
